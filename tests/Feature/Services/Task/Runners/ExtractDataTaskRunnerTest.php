...
class ExtractDataTaskRunnerTest extends AuthenticatedTestCase
{
    ...
    public function run_routes_default_task_to_initialize_with_cached_plan(): void
    {
        ...
        $this->assertArrayHasKey('artifact_id', $classificationProcess->meta);
        $this->assertArrayHasKey('file_id', $classificationProcess->meta);
        $this->assertArrayHasKey('page_number', $classificationProcess->meta);
    ...
    public function classification_operation_completes_successfully(): void
    {
        ...
        // Then: Process completes and output artifact is created
        $taskProcess->refresh();
        $this->assertNotNull($taskProcess->completed_at);

        // Verify output artifact with classification result was created
        $outputArtifacts = $taskProcess->outputArtifacts;
        $this->assertGreaterThan(0, $outputArtifacts->count());
    }
