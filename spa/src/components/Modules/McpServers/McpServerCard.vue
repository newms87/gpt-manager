<template>
	<div class="bg-slate-800/50 rounded-lg p-4 border border-slate-700 hover:border-slate-600 transition-colors">
		<div class="flex-x justify-between items-start mb-3">
			<div class="flex-1">
				<div class="flex-x items-center gap-2 mb-1">
				...
				<div v-if="mcpServer.description" class="text-xs text-slate-500 mt-1">
					{{ mcpServer.description }}
				</div>
			</div>
		</div>

		<div v-if="mcpServer.allowed_tools?.length" class="mt-3 pt-3 border-t border-slate-700">
			<div class="text-xs text-slate-400 mb-2">Available Tools:</div>
		...
		<div v-if="showActions" class="flex-x justify-end gap-2 mt-3 pt-3 border-t border-slate-700">
			<ActionButton
				type="edit"
				color="slate"
				size="sm"
