...
const createAction = dxWorkflowInput.getAction("create", { onFinish: loadWorkflowInputs });
const deleteAction = dxWorkflowInput.getAction("delete", { onFinish: loadWorkflowInputs });
...
async function loadWorkflowInputs() {
    ...
    workflowInputs.value = (await dxWorkflowInput.routes.list({
        fields: { files: { thumb: true }, content: true, teamObject: true },
        filter: filter.value,
        perPage: 10
    })).data;
