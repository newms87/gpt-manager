...
export function layoutTree(
	...
	function getSecondarySize(nodeId: string): number {
	...
	function getPrimarySize(nodeId: string): number {
		const dim = dimensions.get(nodeId);
		return direction === "LR" ? (dim?.width || NODE_WIDTH) : (dim?.height || 100);
	}

	// Memoized leaf count for sorting children by subtree size
	...
	function computeLeafCount(nodeId: string): number {
		...
		const children = childrenMap.get(nodeId) || [];
		const count = children.length === 0 ? 1 : children.reduce((sum, child) => sum + computeLeafCount(child), 0);
	...
	function computeDepths(nodeId: string, depth: number): void {
		...
		maxPrimarySizeAtDepth.set(depth, Math.max(currentMax, getPrimarySize(nodeId)));
	...
	// Store each child's secondary-axis offset relative to its parent
	 ...
	 */
	function layoutSubtree(nodeId: string): Contour {
		const node = nodeMap.get(nodeId);
		if (!node) return { left: [], right: [] };

		const nodeSize = getSecondarySize(nodeId);
		const children = childrenMap.get(nodeId) || [];
		...
		const sortedChildren = [...children].sort((a, b) => computeLeafCount(a) - computeLeafCount(b));

		// Layout each child subtree and get their contours
		const childContours = sortedChildren.map(childId => layoutSubtree(childId));
		...
		for (let i = 1; i < sortedChildren.length; i++) {
			...
			const prevSize = getSecondarySize(sortedChildren[i - 1]);
		...
		const firstChildCenter = childPositions[0] + getSecondarySize(sortedChildren[0]) / 2;
		const lastIdx = sortedChildren.length - 1;
		const lastChildCenter = childPositions[lastIdx] + getSecondarySize(sortedChildren[lastIdx]) / 2;
		...
		for (let i = 0; i < childPositions.length; i++) {
			...
			secondaryOffsets.set(sortedChildren[i], childPositions[i]);
		}
	...
	function assignPositions(nodeId: string, depth: number, parentSecondary: number): void {
		const node = nodeMap.get(nodeId);
		if (!node) return;

		const secondary = parentSecondary + (secondaryOffsets.get(nodeId) || 0);
		const primaryPos = levelPosition.get(depth) || 0;
		...
		for (const child of childrenMap.get(nodeId) || []) {
			assignPositions(child, depth + 1, secondary);
		}
	}
