<template>
	<div v-if="isSelected || selectable" class="schema-object flex items-start flex-nowrap">
		<div
			class="parent-object bg-slate-700 rounded-lg overflow-hidden inline-block w-96 flex-shrink-0"
			:class="selectable ? (isSelected ? selectedClass : notSelectedClass) : ''"
		>
			...
			<div v-if="!readonly || customPropertyNames.length > 0" class="py-2 px-4">
				...
				>
					...
					>
						<ListItemDraggable
							v-if="selectable || subSelection?.children[name]"
