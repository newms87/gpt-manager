	...
	<div class="instruction-template-card bg-white rounded-lg shadow-md border border-slate-200 p-6 hover:shadow-lg transition-all duration-200 w-full">
		...
		<div v-if="isEditingContent" class="mb-4">
			...
			<MarkdownEditor
				...
				:max-length="60000"
				placeholder="Enter your instruction template content..."
		...
		<div v-if="template.files && template.files.length > 0" class="mb-4">
			...
			<div class="flex flex-wrap gap-2">
				<div v-for="file in template.files" :key="file.id" class="flex items-center gap-2 bg-slate-100 rounded px-2 py-1 text-xs">
...
import { ActionButton, EditableDiv, SaveStateIndicator, ShowHideButton, fDate } from "quasar-ui-danx";
import { ref } from "vue";
