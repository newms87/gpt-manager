...
class DuplicateRecordResolver
{
     ...
     *
     * Performs exact comparison of identifying fields to avoid unnecessary LLM calls
     * for obvious matches.
     *
     * @param  array  $extractedData  Extracted identifying data
     * @param  Collection  $candidates  Collection of candidate TeamObjects
     ...
     */
    public function quickMatchCheck(array $extractedData, Collection $candidates): ?TeamObject
    {
        static::logDebug('Performing quick exact match check');

        foreach ($candidates as $candidate) {
            if ($this->isExactMatch($extractedData, $candidate)) {
     ...
     */
    protected function isExactMatch(array $extractedData, TeamObject $candidate): bool
    {
        ...
        foreach ($candidate->attributes as $attribute) {
            $candidateData[$attribute->name] = $attribute->value;
        }

        // Compare all extracted fields
        foreach ($extractedData as $fieldName => $fieldValue) {
            if (!isset($candidateData[$fieldName])) {
                return false; // Field missing in candidate
            }
        ...
        return true; // All fields match exactly
    }
