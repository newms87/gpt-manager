...
class OAuthService
{
    ...
    public function getToken(string $service, ?Team $team = null): ?AuthToken
    {
        ...
        if ($token->willExpireWithin(5)) {
            try {
                ...
                static::logDebug('Failed to refresh token', [
                    'service' => $service,
    ...
    public function getValidToken(string $service, ?Team $team = null): AuthToken
    {
        ...
        if ($needsRefresh && $token->canBeRefreshed()) {
            ...
            try {
                ...
                static::logDebug('Token refresh failed', [
                    'service'     => $service,
                    'team_id'     => $team?->id,
                    'error'       => $e->getMessage(),
    ...
    public function getAuthorizationUrl(string $service, ?string $state = null, ?Team $team = null): string
    {
        ...
        static::logDebug('Generated authorization URL', [
            'service'      => $service,
    ...
    public function exchangeCodeForToken(string $service, string $code): array
    {
        ...
        try {
            static::logDebug('Exchanging authorization code for token', [
                'service'     => $service,
            ...
            if (!$response->successful()) {
                $error = $response->json();
                static::logDebug('Failed to exchange code for token', [
                    'service' => $service,
            ...
            static::logDebug('Successfully exchanged code for token', [
                'service'    => $service,
            ...
            static::logDebug('Exception during token exchange', [
                'service' => $service,
                'error'   => $e->getMessage(),
            ]);
    ...
    public function storeToken(string $service, array $tokenData, ?Team $team = null, array $metadata = []): AuthToken
    {
        ...
        static::logDebug('OAuth token stored successfully', [
            'service'    => $service,
            'team_id'    => $token->team_id,
    ...
    public function refreshToken(string $service, AuthToken $token): AuthToken
    {
        ...
        try {
            static::logDebug('Refreshing OAuth token', [
                'service'    => $service,
                'team_id'    => $token->team_id,
                'expires_at' => $token->expires_at?->toISOString(),
            ]);
            ...
            static::logDebug('Successfully refreshed OAuth token', [
                'service'        => $service,
                'team_id'        => $token->team_id,
            ...
            static::logDebug('Exception during token refresh', [
                'service' => $service,
                'team_id' => $token->team_id,
                'error'   => $e->getMessage(),
            ]);
    ...
    public function validateTokenWithApi(string $service, ?Team $team = null): array
    {
        ...
        // No token exists
        if (!$token) {
            static::logDebug('Token validation failed - no token found', [
                'service' => $service,
                'team_id' => $team?->id,
            ]);
        ...
        if ($token->isExpired()) {
            static::logDebug('Token validation - attempting refresh for expired token', [
                'service' => $service,
                'team_id' => $token->team_id,
            ]);
            ...
            if ($token->canBeRefreshed()) {
                try {
                    ...
                    static::logDebug('Token validation - refresh failed', [
                        'service' => $service,
                        'team_id' => $token->team_id,
                        'error'   => $e->getMessage(),
                    ]);
        ...
        try {
            ...
            if ($isValid) {
                static::logDebug('Token validation successful', [
                    'service' => $service,
                    'team_id' => $token->team_id,
                ]);
            ...
            static::logDebug('Token validation failed - marking as invalid', [
                'service' => $service,
                'team_id' => $token->team_id,
            ]);
            ...
            static::logDebug('Token validation API error', [
                'service' => $service,
                'team_id' => $token->team_id,
                'error'   => $e->getMessage(),
            ]);
    ...
    public function revokeSpecificToken(string $service, AuthToken $token): bool
    {
        ...
        try {
            static::logDebug('Revoking OAuth token', [
                'service' => $service,
                'team_id' => $token->team_id,
            ]);
            ...
            if (isset($config['revoke_url'])) {
                ...
                if (!$response->successful()) {
                    static::logDebug('Failed to revoke token with service (will delete locally)', [
                        'service' => $service,
                        'team_id' => $token->team_id,
                        'status'  => $response->status(),
                    ]);
            ...
            static::logDebug('Successfully revoked OAuth token', [
                'service' => $service,
                'team_id' => $token->team_id,
            ]);
            ...
            static::logDebug('Exception during token revocation', [
                'service' => $service,
                'team_id' => $token->team_id,
                'error'   => $e->getMessage(),
            ]);
